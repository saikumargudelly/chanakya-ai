...
import SessionGoalList from './components/SessionGoalList';
import GoalModal from './components/GoalModal';
import { GoalProvider } from './context/GoalContext';

function App() {
  const [goalModalOpen, setGoalModalOpen] = useState(false);
  const [showGoalTracker, setShowGoalTracker] = useState(window.location.hash === '#goal-tracker');

  React.useEffect(() => {
    const onHashChange = () => setShowGoalTracker(window.location.hash === '#goal-tracker');
              ...
              <section className="grid grid-cols-1 md:grid-cols-5 gap-8 mb-8">
                ...
                <div className="md:col-span-2 flex flex-col gap-8">
                  <div id="budget">
                    ...
                    <SessionGoalList />
                    {/* GoalModal intentionally omitted for session-only goals */}
                  </div>
                </div>
